...
public class FormatLayout extends TextAreaLayout {
	...
	public FormatLayout(LayoutAttrubute attrubute, CharSequence text, TextAreaPaint wp, boolean startSyncTask) {
		...
		mMeasurer = attrubute.buildMeasurer();
		syncTaskEnable = startSyncTask;
		initgenerate();
	}
	...
	protected boolean topToIndex(TextAreaScroller aide, int index, boolean clearPoint) {
		...
		if (result >= 0) {
			int scrollY = aide == null ? 0 : aide.getTextScrollY();
	...
	protected final void startSyncTask() {
		if (syncTaskEnable) {
			mSyncTask = new PointSyncTask(WAIT_TIME);
		}
	}
		...
		if (mLinears.size() > 0) {
			...
			if (index < firstLine.getMinStart()) {
				...
				generateVerticalWithLimit(source, lastLine.getMaxEnd(), length, lastLine.getBottom(),
						aide == null ? 0 : aide.getTextScrollY(), true, true);
			} else {
				handerUpdata = bottomToIndex(aide, index, true);
			}
			checkFirstPageMaxDisplay();
			startSyncTask();
			// Log.e(TAG, "" + mLinears.size() + " " + getHeight() + " " +
			// aide.getTextScrollY() + " "
	...
	public boolean restartLayoutSize(int offsetPosition, int w, int h, TextAreaScroller vh) {
		...
		removeLast(vh == null ? 0 : vh.getTextScrollY() + getHeight(), false);
		return result;
	}
	...
	protected boolean quickToIndex(CharSequence text, int length, int index, TextAreaScroller scroller) {
		float scrollY = scroller == null ? 0 : scroller.getTextScrollY();
		...
		if (scrollY != scroll && scroller != null) {
	...
	protected float getLeadingMarger(Lineable mLine) {
		...
		if (align != null) {
			...
			switch (align) {
			case ALIGN_CENTER:
				float w = (getWidth() - mLine.getMaxSite() - ellipsisW + getPaint().getLetterSpacing()) / 2;
				return w < 0 ? 0 : w;
			case ALIGN_RIGHT:
	...
	private class PointSyncTask extends SyncTask {
		...
		void execute() {
			...
			Measurer bufferMeasurer = newMeasurer();
			Lineable mLine = bufferMeasurer.installNewLineable();
			if (maxEnd < length) {
				while (!isCancel && !mPoints.isLimit()) {
					try {
 ...
 * @author Maizer/麦泽
 */
abstract class SyncTask extends Thread {

	private static final String TAG = SyncTask.class.getCanonicalName();
	...
	public final void run() {
		synchronized (this) {
			...
			if (!isCancel) {
				if (BuildConfig.DEBUG)
					Log.e(TAG, "SyncTask Start!");
				execute();
			}
			if (BuildConfig.DEBUG)
				Log.e(TAG, "SyncTask Exit!");
		}
	}
}
